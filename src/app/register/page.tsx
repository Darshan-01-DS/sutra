'use client'
// src/app/register/page.tsx — Split-screen register page
    ...
    <div className="auth-split">
      ...
      <div className="auth-form-side">
        <div className="auth-form-inner">
          ...
          <p className="auth-form-sub">Your thinking system starts here.</p>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ fontSize: 11, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 5 }}>Name</div>
            <input className="auth-input" type="text" placeholder="Your name" value={name} onChange={e => { setName(e.target.value); setError('') }} required />
            ...
            <input className="auth-input" type="password" placeholder="Min. 8 characters" value={password} onChange={e => { setPassword(e.target.value); setError('') }} required minLength={8} />
            <button type="submit" className="auth-btn-primary" disabled={loading} style={{ marginTop: 4 }}>
              {loading ? 'Creating account…' : 'Create my account'}
            </button>
          </form>

          <p className="auth-footer-text">Already have an account? <Link href="/login" className="auth-link">Sign in →</Link></p>
        </div>
      </div>
    </div>
  )
